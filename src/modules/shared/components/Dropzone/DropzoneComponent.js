...
const images_extensions = ['png', 'jpg', 'jpeg', 'gif', 'jfif', 'bmp', 'psd', 'tiff', 'exif', 'raw'];
...
const DropzoneComponent = forwardRef((props, ref) => {
    ...
    useEffect(() => {
        setShowFiles(false);
        setShowLoader(true);
        let readedFiles = props.files ? props.files : [];
        setFilesAccepted(readedFiles);
        setTimeout(() => {
            setShowLoader(false);
    ...
    const files = filesAccepted.map((file, index) => {
        ...
        return (
            <Grid key={file.path} item>
                <Paper elevation={0}>
                    {is_img ? <img src={url} width={50} /> : getIcon(ext)}
                    <Paper style={{ padding: 10 }}>{file.path} <IconButton onClick={() => resetFiles(index)}> <DeleteForeverIcon style={{ fontSize: 20, color: '#333333' }} /></IconButton></Paper>
                </Paper>
            </Grid>
        );
    })
    ...
    return (
        <section className={styles.section}>
            <div style={{ cursor: 'pointer' }} {...getRootProps({ className: 'dropzone' })}>
                ...
                <BackupIcon color={'black'} style={{ fontSize: 90 }} />
            </div>
            {
                showLoader &&
